...
def plot_labels(labels, save_dir=Path(""), loggers=None):
    ...
    # Save numeric label data to text file
    labels_txt_path = save_dir / "labels.txt"
    x.to_csv(labels_txt_path, sep=",", index=False, float_format="%.6f")
...
def plot_results(start=0, stop=0, bucket="", id=(), labels=(), save_dir=""):
    ...
    for fi, f in enumerate(files):
        try:
            results = np.loadtxt(
                f, usecols=[2, 3, 4, 9, 10, 13, 14, 15, 11, 12], ndmin=2
            ).T
            n = results.shape[1]  # number of rows
            x = range(start, min(stop, n) if stop else n)
            for i in range(10):
