...
class ConfusionMatrix:
    ...
    def plot(self, save_dir="", names=()):
        try:
            ...
            txt_path = Path(save_dir) / "confusion_matrix.txt"

            TP, FN = array[0, 0], array[0, 1]
            FP, TN = array[1, 0], array[1, 1]
...
def plot_pr_curve(px, py, ap, save_dir=".", names=()):
    ...
    txt_path = Path(save_dir) / "precision_recall_curve.txt"
    with open(txt_path, "w") as f:
        f.write("# Recall, Precision\n")
        np.savetxt(f, np.column_stack((px, py.mean(1))), fmt="%.6f", delimiter=", ")
